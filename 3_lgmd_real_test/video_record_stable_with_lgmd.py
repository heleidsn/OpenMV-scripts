...
2023-03-29 新校区实验 能够实现部分避障功能
2023-03-30 新校区实验 准备测试自主飞行，但是由于无法获取位置，测试失败
2023-03-31 新校区实验 测试自主飞行功能；30号原因排查：解锁后无法获取mavlink ID 32 local_position 比较神奇，最终通过修改px4固件解决
           改变波特率为921600
           修改PX4固件中on-board模式下mavlink消息发送频率，姿态、RC为20Hz，位置为50Hz，通过强行多发送数据来确保解锁后仍然可以获取
           具体原因为什么解锁前能够获取位置，解锁后无法获取位置，还需要进一步排查
           最终两次飞行多次试验，表明还是能够实现自主飞行，具有一定的成功率
